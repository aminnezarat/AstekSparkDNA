...
object AstekSeqSimpleJob {
 ...
 def main(args: Array[String]) {
   ...
   //val sc = new  SparkContext("spark://MarekNotebook:7077", "sparkseq", System.getenv("SPARK_HOME"),  Seq(System.getenv("ADD_JARS")))
   // , List("target/scala-2.9.3/sparkseq_2.9.3-0.1.jar","/opt/hadoop-classpath/hadoop-bam-6.1-SNAPSHOT.jar","/opt/hadoop-classpath/picard-1.106.jar","/opt/hadoop-classpath/sam-1.106.jar", "/opt/hadoop-classpath/variant-1.106.jar","/opt/hadoop-classpath/tribble-1.106.jar","/opt/hadoop-classpath/commons-jexl-2.1.1.jar"))
    ...
    /*val seqAnalysisCase = new AstekSeqAnalysis(sc,"hdfs://hadoop-name001:9000/HG/16MB/HG00096.mapped.ILLUMINA.bwa.GBR.low_coverage.20120522.bam",1,"NULL",1,40)
    ...
    */
    ...
    //val CMTest = new AstekSeqCvM2STest()
    ...
    val seqAnalysisCase = new AstekSeqAnalysis(sc,pathFam1+"/Case/Sample_25_sort.bam",25,1,numTasks)
    ...
    val seqAnalysisControl = new AstekSeqAnalysis(sc,pathFam1+"/Control/Sample_26_sort.bam",26,1,numTasks)
    for(i<-controlIdFam1){
