 ...
 */
package seq.am.yz.astekdna.rsparkseq

import org.apache.spark.SparkContext
import org.apache.spark.rdd.RDD
import seq.am.yz.astekdna.util.AstekSeqRegType._
import seq.am.yz.astekdna.conversions.AstekSeqConversions
import seq.am.yz.astekdna.seqAnalysis.AstekSeqAnalysis
import seq.am.yz.astekdna.util.AstekSeqRegType
 ...
 */
class RAstekSeqAnalysis(iSC: SparkContext, iBAMFile: String, iSampleId: Int, iNormFactor: Double, iReduceWorkers: Int = 8)
  extends AstekSeqAnalysis(iSC, iBAMFile, iSampleId, iNormFactor, iReduceWorkers) {

  /**
   * Get all samples of the RAstekSeqAnalysis object
    *
   ...
   * Get feature counts for a given list of regions of interest
    *
    * @param iGenExons A Spark broadcast variable created from BED file that is transformed using AstekSeqConversions.BEDFileToHashMap
  ...
  def getRGeneCoverage(geneArray: Array[String]): Array[(String, Array[Int])] = {
    ...
    val regionCovFilterRDD = regionCovRDD
      .filter(r => geneArray.contains(AstekSeqConversions.ensemblRegionIdToExonId(r._1, Gene)))
