 ...
 */
package seq.am.yz.astekdna.differentialExpression

import org.apache.spark.{HashPartitioner, RangePartitioner, SparkContext, broadcast}
import org.apache.spark.SparkContext._
import org.apache.spark.rdd._

import scala.collection.mutable.ArrayBuffer
import seq.am.yz.astekdna.statisticalTests._
...
import seq.am.yz.astekdna.permutationTests._
import seq.am.yz.astekdna.statisticalTests._
import seq.am.yz.astekdna.conversions.AstekSeqConversions
import seq.am.yz.astekdna.permutationTests.AstekSeqAdaptivePermutTest
import seq.am.yz.astekdna.seqAnalysis.AstekSeqAnalysis
import seq.am.yz.astekdna.statisticalTests.{AstekSeqCvM2STest, AstekSeqKS2STest, AstekSeqStatisticalTest, AstekSeqStats}
 ...
 * @param iSeqAnalCase AstekSeqAnalysis object for case samples.
 * @param iSeqAnalControl AstekSeqAnalysis object for control samples.
 ...
 */
class AstekSeqDiffExpr(iSC: SparkContext, iSeqAnalCase: AstekSeqAnalysis, iSeqAnalControl: AstekSeqAnalysis, iBEDFile: String, iChr: String = "*",
  ...
  private val genExonsMapB = iSC.broadcast(AstekSeqConversions.BEDFileToHashMap(iSC, confDir + iBEDFile))
  private val genExonsMapLookupB = iSC.broadcast(AstekSeqConversions.BEDFileToHashMapGeneExon(iSC, confDir + iBEDFile))

  private def groupSeqAnalysis(iSeqAnalysis: AstekSeqAnalysis, iSampleNum: Int): RDD[(Long, Iterable[Int])] = {
  ...
  private def computeTwoSampleCvMTest(iSeqCC: RDD[(Long, (Seq[Int], Seq[Int]))]): RDD[((Int, Double), (Long, Double, Double, Double))] = {
    ...
    val twoSampleTests = iSeqCC
      .map(r => (r._1, r._2, AstekSeqCvM2STest.getTestStatistics(r._2._1, r._2._2)))
      .map(r => ((r._1), (r._2, r._3, AstekSeqCvM2STest.getPValue(r._3, cmDistTableB),
      AstekSeqStats.mean(r._2._1) / AstekSeqStats.mean(r._2._2), AstekSeqStats.mean(r._2._1), AstekSeqStats.mean(r._2._2))))
  ...
  private def mapRegionsToExons(r: (Double, Int, Long, Double)): ArrayBuffer[(Double, Int, (String, Int), Double, String, String, Double)] = {
    ...
    val reg = (r._1, r._2, AstekSeqConversions.idToCoordinates(r._3), r._4)
  ...
  def findCandRegions(iCoalesceRegDiffPVal: Boolean = true): (scala.collection.mutable.HashMap[String, Array[ArrayBuffer[(String, String, Int, Int)]]],
    ...
    val seqFilterCC = seqJointCC.filter(r => (AstekSeqStats.mean(r._2._1.toArray) >= iMinCoverage || AstekSeqStats.mean(r._2._2.toArray) >= iMinCoverage))
  ...
  HashMap[String, Array[scala.collection.mutable.ArrayBuffer[(String, String, Int, Int)]]]]): RDD[((String), Double, Double)] = {
    val seqRegCovCase = iSeqAnalCase.getCoverageRegion(iRegions).map(r => ((AstekSeqConversions.stripSampleID(r._1), r._2)))
      .groupByKey()
      .mapValues(c => if ((caseSampleNum - c.size) > 0) (c ++ ArrayBuffer.fill[Int](caseSampleNum - c.size)(0)) else (c))
    val seqRegCovControl = iSeqAnalControl.getCoverageRegion(iRegions).map(r => ((AstekSeqConversions.stripSampleID(r._1), r._2)))
      .groupByKey()
    ...
    val permTestRegionD = jointRegion.map {
      r =>
        val statTests = Array[AstekSeqStatisticalTest](AstekSeqCvM2STest, AstekSeqKS2STest)
        val permTest = new AstekSeqAdaptivePermutTest(iNPermut = 10000, iStatTests = statTests, r._2._1.toSeq, r._2._2.toSeq)
        (AstekSeqConversions.ensemblRegionIdToExonId(r._1), permTest.getPvalue(), AstekSeqStats.mean(r._2._1.toSeq) / AstekSeqStats.mean(r._2._2.toSeq), r._2._1, r._2._2)
    }
  ...
  def getDistExonCandidates(): scala.collection.mutable.HashMap[String, Array[ArrayBuffer[(String, String, Int, Int) /*(GeneId,ExonId,Start,End,tId)*/ ]]] = {
    ...
    val exonCandHashMap = AstekSeqConversions.exonsToHashMap(exonCand)
  ...
  def getRegionCandidates(): scala.collection.mutable.HashMap[String, Array[ArrayBuffer[(String, String, Int, Int) /*(GeneId,ExonId,Start,End)*/ ]]] = {
    ...
    val unRegionCandHashMap = AstekSeqConversions.exonsToHashMap(unRegionCand)
