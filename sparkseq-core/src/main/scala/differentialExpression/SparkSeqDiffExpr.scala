...
class SparkSeqDiffExpr(iSC: SparkContext, iSeqAnalCase: SparkSeqAnalysis, iSeqAnalControl: SparkSeqAnalysis, iChr: String,
  ...
  private def joinSeqAnalysis(iSeqAnalysis1: RDD[(Long, Seq[Int])], iSeqAnalysis2: RDD[(Long, Seq[Int])]): RDD[(Long, (Seq[Int], Seq[Int]))] = {
    val leftSeqJoint = iSeqAnalysis1.leftOuterJoin(iSeqAnalysis2)
    val rightSeqJoint = iSeqAnalysis1.rightOuterJoin(iSeqAnalysis2)
